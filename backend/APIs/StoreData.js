...
storeApp.post("/add", async (req, res) => {
  try {
    ...
    res.status(500).send({ status: 500, message: "Error adding medicine", error });
  }
});
...
storeApp.put("/:id", async (req, res) => {
  try {
    ...
    if (!updatedMedicine || Object.keys(updatedMedicine).length === 0) {
      return res.status(400).send({ status: 400, message: "No data provided to update" });
    }
    ...
    if (result.modifiedCount >= 1) {
      res.status(200).send({ status: 200, message: "Medicine updated successfully" });
    } else {
    ...
    res.status(500).send({ status: 500, message: "Error updating medicine", error });
  }
});
...
storeApp.delete("/:id", async (req, res) => {
  try {
    ...
    if (result.deletedCount >= 1) {
      res.status(200).send({ status: 200, message: "Medicine deleted successfully" });
    } else {
      res.status(200).send({ status: 200, message: "Medicine not deleted" });
    }
  } catch (error) {
    console.error("Error deleting medicine:", error); // Updated error message
    res.status(500).send({ status: 500, message: "Error deleting medicine", error });
  }
});
